...
| `GOOSE_SYSTEM_PROMPT_FILE_PATH` | Path to a file whose contents **REPLACE** goose's built-in agent prompt. This is the replacing counterpart to `goose run --system <TEXT>`, which only APPENDS its text under a "# Additional Instructions:" heading — set this when you want the agent's system prompt to be yours alone. The file is rendered as a template (so `{{current_date_time}}` is interpolated) and read once, at session start. A configured path that cannot be read is a fatal error rather than a silent fallback to the built-in prompt. Note that the tool surface is unaffected: tools travel in the request's own `tools` field, not in the system prompt. | Absolute or relative file path | Unset (the built-in prompt is used) |
