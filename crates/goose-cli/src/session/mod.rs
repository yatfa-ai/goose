...
impl CliSession {
    ...
    async fn run_interactive(&mut self, prompt: Option<String>) -> Result<()> {
        ...
        // still streams to the attached client over the loopback WebSocket; this only
...
/// run-side serve tap task so turns from an attached client read like a chat:
...
/// the attached client is already receiving over the loopback WebSocket.
        ...
        goose::acp::AcpTurnEvent::PromptReceived { text, .. } => {
            ...
            *thinking_header_shown = false;
            ...
            if !trimmed.is_empty() {
                ...
                output::render_text(&format!("acp ▸ {trimmed}"), Some(Color::Cyan), false);
            }
        }
        goose::acp::AcpTurnEvent::StreamEvent(event) => match *event {
