...
//! turns started by an attached client (`session/prompt` arriving over the
//! loopback ACP WebSocket) render in the terminal the same way user-typed input
//! does, instead of being invisible while the ACP handler streams the reply
//! straight back to that client.
...
pub enum AcpTurnEvent {
    /// A `session/prompt` arrived. `text` is the attached client's prompt rendered as
    ...
    /// through the same path as a user-initiated turn.  Boxed to keep the enum
    /// small: `AgentEvent` dwarfs every other variant and this one is sent per
    /// stream tick.
